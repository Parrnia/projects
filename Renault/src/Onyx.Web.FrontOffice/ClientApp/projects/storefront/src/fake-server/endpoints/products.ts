...
import { CheckVehicleFilterBuilder } from '../filters/check-vehicle-filter-builder';
...
function getProducts(shift: number, categorySlug: string|null = null): Product[] {
    switch (categorySlug) {
        case 'tires-wheels':
...
export function getProductsList(options?: GetProductsListOptions): Observable<ProductsList> {
    ...
    let categoryFilterBuilder = new CategoryFilterBuilder();
    categoryFilterBuilder.setNameAndSlug('category', 'دسته بندی ها');
    let vehicleFilterBuilder = new VehicleFilterBuilder();
    vehicleFilterBuilder.setNameAndSlug('vehicle', 'خودرو');
    let rangeFilterBuilder = new RangeFilterBuilder();
    rangeFilterBuilder.setNameAndSlug('price', 'قیمت');
    let checkFilterBuilder = new CheckVehicleFilterBuilder();
    checkFilterBuilder.setNameAndSlug('brand', 'برند');
    let radioFilterBuilder = new RadioFilterBuilder();
    radioFilterBuilder.setNameAndSlug('discount', 'با تخفیف');
    let ratingFilterBuilder = new RatingFilterBuilder();
    ratingFilterBuilder.setNameAndSlug('rating', 'امتیاز');
    let colorFilterBuilder = new ColorFilterBuilder();
    colorFilterBuilder.setNameAndSlug('color', 'رنگ');
