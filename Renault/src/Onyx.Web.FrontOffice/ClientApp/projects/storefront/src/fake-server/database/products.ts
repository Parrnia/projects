...
import { Product, ProductAttribute, ProductAttributeOption } from '../../app/interfaces/product';
...
function makeProducts(defs: ProductDef[]): Product[] {
    return defs.map(def => {
        ...
        const categories: ShopCategory[] = categorySlugs
            ...
            .filter((x): x is ShopCategory => x !== undefined)
        ...
        return {
            ...
            customFields: {},
            attributeOptions : [],
            selectedAttributeOption : new ProductAttributeOption(),
            sevenId: ''
        };
    });
}
