...
public record ExportExcelModelsQuery() : IRequest<byte[]>
{
...
public class ExportExcelModelsQueryHandler : IRequestHandler<ExportExcelModelsQuery, byte[]>
{
    private readonly IApplicationDbContext _context;
    private readonly IExportServices _exportServices;


    public ExportExcelModelsQueryHandler(IApplicationDbContext context, IExportServices exportServices)
    {
        _context = context;
        _exportServices = exportServices;
    }

    public async Task<byte[]> Handle(ExportExcelModelsQuery request, CancellationToken cancellationToken)
    {
        var models = _context.Models
            .OrderBy(x => x.Name);
        
        var selectedProperties = new List<Expression<Func<Model, object>>?>()
        {
            c => c.LocalizedName,
            c => c.Name,
            c => c.Family,
