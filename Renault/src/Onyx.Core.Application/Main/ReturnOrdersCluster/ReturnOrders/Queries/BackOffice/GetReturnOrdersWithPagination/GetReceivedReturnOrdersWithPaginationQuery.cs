...
public record GetReceivedReturnOrdersWithPaginationQuery : IRequest<FilteredReturnOrderDto>
{
...
public class GetReceivedReturnOrdersWithPaginationQueryHandler : IRequestHandler<GetReceivedReturnOrdersWithPaginationQuery, FilteredReturnOrderDto>
{
    private readonly IApplicationDbContext _context;
    private readonly IMapper _mapper;

    public GetReceivedReturnOrdersWithPaginationQueryHandler(IApplicationDbContext context, IMapper mapper)
    {
        _context = context;
        _mapper = mapper;
    }

    public async Task<FilteredReturnOrderDto> Handle(GetReceivedReturnOrdersWithPaginationQuery request, CancellationToken cancellationToken)
    {
        var returnOrders = _context.ReturnOrders.OrderBy(c => c.CreatedAt).Where(c => c.ReturnOrderStateHistory.OrderBy(e => e.Created).Last().ReturnOrderStatus == ReturnOrderStatus.Received);
