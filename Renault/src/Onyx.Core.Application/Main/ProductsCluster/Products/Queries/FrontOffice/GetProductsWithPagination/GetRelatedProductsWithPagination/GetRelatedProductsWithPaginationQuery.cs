...
public record GetRelatedProductsWithPaginationQuery : IRequest<PaginatedList<RelatedProductWithPaginationDto>>
{
    public int PageNumber { get; init; } = 1;
    public int PageSize { get; init; } = 10;
    public CustomerTypeEnum CustomerTypeEnum { get; set; }
}
public class GetRelatedProductsWithPaginationQueryHandler : IRequestHandler<GetRelatedProductsWithPaginationQuery, PaginatedList<RelatedProductWithPaginationDto>>
{
    private readonly ISharedProductQueryHelperMethods _productQueryHelperMethods;
    private readonly IMapper _mapper;

    public GetRelatedProductsWithPaginationQueryHandler(IMapper mapper, ISharedProductQueryHelperMethods productQueryHelperMethods)
    {
        _mapper = mapper;
        _productQueryHelperMethods = productQueryHelperMethods;
    }

    public async Task<PaginatedList<RelatedProductWithPaginationDto>> Handle(GetRelatedProductsWithPaginationQuery request, CancellationToken cancellationToken)
    {
        var products = _productQueryHelperMethods.GetProducts(c => c.IsActive);

        var helperResult = await products.ProjectTo<RelatedProductWithPaginationModelHelperDto>(_mapper.ConfigurationProvider)
        ...
        var finalResult = new PaginatedList<RelatedProductWithPaginationDto>(result, helperResult.TotalCount, request.PageNumber, request.PageSize);
        return finalResult;
    }
}
