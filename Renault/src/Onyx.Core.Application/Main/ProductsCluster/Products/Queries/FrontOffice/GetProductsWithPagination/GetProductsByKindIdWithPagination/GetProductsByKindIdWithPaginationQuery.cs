...
public record GetProductsByKindIdWithPaginationQuery : IRequest<PaginatedList<ProductByKindIdWithPaginationDto>>
{
...
public class GetProductsWithKindIdWithPaginationQueryHandler : IRequestHandler<GetProductsByKindIdWithPaginationQuery, PaginatedList<ProductByKindIdWithPaginationDto>>
{
    private readonly ISharedProductQueryHelperMethods _productQueryHelperMethods;
    private readonly IMapper _mapper;

    public GetProductsWithKindIdWithPaginationQueryHandler(IMapper mapper, ISharedProductQueryHelperMethods productQueryHelperMethods)
    {
        _mapper = mapper;
        _productQueryHelperMethods = productQueryHelperMethods;
    }

    public async Task<PaginatedList<ProductByKindIdWithPaginationDto>> Handle(GetProductsByKindIdWithPaginationQuery request, CancellationToken cancellationToken)
    {
        var products = _productQueryHelperMethods.GetProducts(c => c.IsActive)
            .Where(x => x.Product.Kinds.Any(y => y.Id == request.KindId));

        var helperResult = await products.ProjectTo<ProductByKindIdWithPaginationModelHelperDto>(_mapper.ConfigurationProvider)
        ...
        var finalResult = new PaginatedList<ProductByKindIdWithPaginationDto>(result, helperResult.TotalCount, request.PageNumber, request.PageSize);
        return finalResult;
    }
}
