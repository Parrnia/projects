...
public record GetLatestProductsWithPaginationQuery : IRequest<PaginatedList<LatestProductWithPaginationDto>>
{
    public int PageNumber { get; init; } = 1;
    public int PageSize { get; init; } = 10;
    public CustomerTypeEnum CustomerTypeEnum { get; set; }
}
public class GetLatestProductsWithPaginationQueryHandler : IRequestHandler<GetLatestProductsWithPaginationQuery, PaginatedList<LatestProductWithPaginationDto>>
{
    private readonly IMapper _mapper;
    private readonly ISharedProductQueryHelperMethods _productQueryHelperMethods;

    public GetLatestProductsWithPaginationQueryHandler(IMapper mapper, ISharedProductQueryHelperMethods productQueryHelperMethods)
    {
        _mapper = mapper;
        _productQueryHelperMethods = productQueryHelperMethods;
    }

    public async Task<PaginatedList<LatestProductWithPaginationDto>> Handle(GetLatestProductsWithPaginationQuery request, CancellationToken cancellationToken)
    {
        var products = _productQueryHelperMethods.GetProducts(c => c.IsLatest);


        var helperResult = await products.ProjectTo<LatestProductWithPaginationModelHelperDto>(_mapper.ConfigurationProvider)
        ...
        var finalResult = new PaginatedList<LatestProductWithPaginationDto>(result, helperResult.TotalCount, request.PageNumber, request.PageSize);
        return finalResult;
    }
}
