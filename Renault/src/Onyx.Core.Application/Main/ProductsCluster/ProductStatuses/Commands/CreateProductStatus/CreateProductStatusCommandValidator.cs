...
public class CreateProductStatusCommandValidator : AbstractValidator<CreateProductStatusCommand>
{
    private readonly IApplicationDbContext _context;

    public CreateProductStatusCommandValidator(IApplicationDbContext context)
    {
        _context = context;
        ...
        RuleFor(v => v.LocalizedName)
            .MustAsync(BeUniqueLocalizedName).WithMessage("وضعیت محصولی با این نام فارسی موجود است")
            .NotEmpty().WithMessage("نام فارسی اجباریست")
            .MaximumLength(50).WithMessage("نام فارسی نباید بیشتر از 50 کاراکتر باشد");
        RuleFor(v => v.Name)
            .MustAsync(BeUniqueName).WithMessage("وضعیت محصولی با این نام لاتین موجود است")
