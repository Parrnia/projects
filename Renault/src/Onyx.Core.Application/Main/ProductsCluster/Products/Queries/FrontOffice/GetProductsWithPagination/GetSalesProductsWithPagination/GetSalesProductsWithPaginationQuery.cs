...
public record GetSalesProductsWithPaginationQuery : IRequest<PaginatedList<SalesProductWithPaginationDto>>
{
    public int PageNumber { get; init; } = 1;
    public int PageSize { get; init; } = 10;
    public CustomerTypeEnum CustomerTypeEnum { get; set; }

    public class GetSalesProductsWithPaginationQueryHandler : IRequestHandler<GetSalesProductsWithPaginationQuery,
        PaginatedList<SalesProductWithPaginationDto>>
    {
        private readonly ISharedProductQueryHelperMethods _productQueryHelperMethods;
        private readonly IMapper _mapper;

        public GetSalesProductsWithPaginationQueryHandler(IMapper mapper, ISharedProductQueryHelperMethods productQueryHelperMethods)
        {
            _mapper = mapper;
            _productQueryHelperMethods = productQueryHelperMethods;
        }

        public async Task<PaginatedList<SalesProductWithPaginationDto>> Handle(
            GetSalesProductsWithPaginationQuery request, CancellationToken cancellationToken)
        {
            var products = _productQueryHelperMethods.GetProducts(c => c.IsSale);

            var helperResult = await products.ProjectTo<SalesProductWithPaginationModelHelperDto>(_mapper.ConfigurationProvider)
            ...
            var finalResult = new PaginatedList<SalesProductWithPaginationDto>(result, helperResult.TotalCount, request.PageNumber, request.PageSize);
            return finalResult;
        }
    }
}
