...
import { FilteredTagDto, TagsClient } from 'src/app/web-api-client';
import { SortColumn, SortDirection } from './tag-sortable.directive';
...
export class TagGridService {
  private _loading$ = new BehaviorSubject<boolean>(true);
  private _search$ = new Subject<void>();
  private _tags$ = new BehaviorSubject<TagModel[]>([]);
  ...
  constructor(private pipe: DecimalPipe, public client: TagsClient) {
    debugger;
    this._search$.pipe(
  ...
  private _search(): Observable<SearchResult> {
    ...
    debugger;
    return this.client.getTagsWithPagination(page, pageSize, sortColumn, sortDirection, searchTerm)
      .pipe(
        map((result: FilteredTagDto) => {
          if (!result) {
            return { tags: [], total: 0 };
          }
          ...
          let end = result.tags!.length + (page-1) * pageSize;
          this.endIndex = Math.min(end, page * pageSize);
          this.pageSize = pageSize;
          let searchResult = new SearchResult();
          searchResult.tags = tags as any;
