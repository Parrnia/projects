...
public record GetProductsByKindIdQuery : IRequest<List<ProductByKindIdDto>>
{
    public int KindId { get; init; }
    public CustomerTypeEnum CustomerTypeEnum { get; set; }
}

public class GetProductsByKindIdQueryHandler : IRequestHandler<GetProductsByKindIdQuery, List<ProductByKindIdDto>>
{
    private readonly IMapper _mapper;
    private readonly ISharedProductQueryHelperMethods _productQueryHelperMethods;


    public GetProductsByKindIdQueryHandler(IMapper mapper, ISharedProductQueryHelperMethods productQueryHelperMethods)
    {
        _mapper = mapper;
        _productQueryHelperMethods = productQueryHelperMethods;
    }

    public async Task<List<ProductByKindIdDto>> Handle(GetProductsByKindIdQuery request, CancellationToken cancellationToken)
    {
        var products = _productQueryHelperMethods.GetProducts(c => c.IsActive)
            .Where(p => p.Product.Kinds.Select(c => c.Id).Contains(request.KindId));

        var helperResult = await products.ProjectTo<ProductByKindIdModelHelperDto>(_mapper.ConfigurationProvider)
