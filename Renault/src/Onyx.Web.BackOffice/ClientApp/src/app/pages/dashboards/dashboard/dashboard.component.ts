...
import { RequestLogsClient, SyncDataClient } from 'src/app/web-api-client';
...
export class DashboardComponent implements OnInit {
  ...
  constructor(
    ...
    private requestLogsClient : RequestLogsClient
  ) {
    var dt = new Date();
    dt.setDate(dt.getDate() + 15);
    this.currentDate = { from: Date.now(), to: dt }
  }
  ...
  ngOnInit(): void {
    ...
    this.breadCrumbItems = [
      { label: 'صفحه اصلی' },
      { label: 'صفحه اصلی', active: true }
    ];

    if (localStorage.getItem('toast')) {
      this.toastService.showSuccess('Logged in Successfull.', { classname: 'bg-success text-center text-white', delay: 5000 });
  ...
  resendAllFailedCancelOrderRequests(){
    this.requestLogsClient.resendAllCancelOrderRequests().subscribe({
      next : (res) => {
        if(res){
          this.toastService.showSuccess('عملیات با موفقیت انجام شد');
        } else {
          this.toastService.showError('عملیات دچار مشکل شد');
        }
      }
    })
  ...
  syncPrice(){
    this.syncDataClient.syncPrices().subscribe({
      next : (res) => {
        if(res){
          this.toastService.showSuccess('عملیات با موفقیت انجام شد');
        } else {
  ...
  syncCount(){
    this.syncDataClient.syncCounts().subscribe({
      next : (res) => {
        if(res){
          this.toastService.showSuccess('عملیات با موفقیت انجام شد');
        } else {
  ...
  syncPriceAndCount(){
    debugger;
    this.syncDataClient.syncCountsAndPrices().subscribe({
      next : (res) => {
        if(res){
          this.toastService.showSuccess('عملیات با موفقیت انجام شد');
        } else {
