import { Component, HostBinding, Input, OnDestroy, OnInit } from '@angular/core';
import { BehaviorSubject, merge, of, Subject } from 'rxjs';
import { FormControl } from '@angular/forms';
import { ReviewsList } from '../../../../interfaces/list';
import { ShopApi } from '../../../../api';
import { UrlService } from '../../../../services/url.service';
import { distinctUntilChanged, switchMap, takeUntil } from 'rxjs/operators';
import { ProductmapperService } from 'projects/storefront/src/app/mapServieces/productsCluster/productmapper.service';
import { ProductsClient, ReviewsClient } from 'projects/storefront/src/app/web-api-client';
import { ReviewmapperService } from 'projects/storefront/src/app/mapServieces/productsCluster/reviewmapper.service';

@Component({
    selector: 'app-reviews-list',
    templateUrl: './reviews-list.component.html',
    styleUrls: ['./reviews-list.component.scss'],
})
export class ReviewsListComponent implements OnInit, OnDestroy {
    private destroy$: Subject<void> = new Subject<void>();
    private productId$: BehaviorSubject<number|null> = new BehaviorSubject<number|null>(null);

    currentPage: FormControl = new FormControl(1);

    list: ReviewsList|null = null;

    @Input() set productId(value: number) {
        if (value !== this.productId$.value) {
            this.currentPage.setValue(1, { emitEvent: false });
            this.productId$.next(value);
        }
    }

    @HostBinding('class.reviews-list') classReviewsList = true;

    constructor(
        private reviewsClient : ReviewsClient,
        private reviewmapperService : ReviewmapperService,
        private shop : ShopApi,
        public url: UrlService,
    ) { }

    ngOnInit(): void {
         
        this.productId$.pipe(
            switchMap(productId => {
                if (!productId) {
                    return of(null);
                }

                return merge(
                    of(this.currentPage.value),
                    this.currentPage.valueChanges,
                ).pipe(
                    distinctUntilChanged(),
                    switchMap(page => this.reviewsClient.getReviewsByProductIdWithPagination(page,8,productId).pipe(switchMap((res => ([this.reviewmapperService.getProductReviews(res.items ?? [],page,8)])))))
                );
            }),
        ).pipe(
            takeUntil(this.destroy$),
        ).subscribe(x =>  x?.subscribe({
            next : (e) => {
                 
                this.list = e;
            }
        }
        ));
    }

    ngOnDestroy(): void {
        this.destroy$.next();
        this.destroy$.complete();
    }

    reload(): void {
        this.productId$.next(this.productId$.value);
    }
}
