...
public record GetRegisteredOrdersWithPaginationQuery : IRequest<FilteredOrderDto>
{
...
public class GetRegisteredOrdersWithPaginationQueryHandler : IRequestHandler<GetRegisteredOrdersWithPaginationQuery, FilteredOrderDto>
{
    private readonly IApplicationDbContext _context;
    private readonly IMapper _mapper;

    public GetRegisteredOrdersWithPaginationQueryHandler(IApplicationDbContext context, IMapper mapper)
    {
        _context = context;
        _mapper = mapper;
    }

    public async Task<FilteredOrderDto> Handle(GetRegisteredOrdersWithPaginationQuery request, CancellationToken cancellationToken)
    {
        var orders = _context.Orders.OrderBy(c => c.CreatedAt).Where(c => c.OrderStateHistory.OrderBy(e => e.Created).Last().OrderStatus == OrderStatus.OrderRegistered);
