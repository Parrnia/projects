...
public record GetProductsByProductCategoryIdQuery : IRequest<List<ProductByProductCategoryIdDto>>
{
    public int ProductCategoryId { get; init; }
    public CustomerTypeEnum CustomerTypeEnum { get; set; }

}
public class GetProductsByProductCategoryIdQueryHandler : IRequestHandler<GetProductsByProductCategoryIdQuery, List<ProductByProductCategoryIdDto>>
{
    ...
    public GetProductsByProductCategoryIdQueryHandler(IMapper mapper, ISharedProductQueryHelperMethods productQueryHelperMethods, IApplicationDbContext context)
    {
        _mapper = mapper;
    ...
    public async Task<List<ProductByProductCategoryIdDto>> Handle(GetProductsByProductCategoryIdQuery request, CancellationToken cancellationToken)
    {
        var products = _productQueryHelperMethods.GetProducts(c => c.IsActive)
            .Where(p => p.Product.ProductCategoryId == request.ProductCategoryId);


        var helperResult = await products.ProjectTo<ProductByProductCategoryIdModelHelperDto>(_mapper.ConfigurationProvider)
