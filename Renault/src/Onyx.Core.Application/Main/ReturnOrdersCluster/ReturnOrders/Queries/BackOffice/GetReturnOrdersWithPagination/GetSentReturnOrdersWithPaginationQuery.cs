...
public record GetSentReturnOrdersWithPaginationQuery : IRequest<FilteredReturnOrderDto>
{
...
public class GetSentReturnOrdersWithPaginationQueryHandler : IRequestHandler<GetSentReturnOrdersWithPaginationQuery, FilteredReturnOrderDto>
{
    private readonly IApplicationDbContext _context;
    private readonly IMapper _mapper;

    public GetSentReturnOrdersWithPaginationQueryHandler(IApplicationDbContext context, IMapper mapper)
    {
        _context = context;
        _mapper = mapper;
    }

    public async Task<FilteredReturnOrderDto> Handle(GetSentReturnOrdersWithPaginationQuery request, CancellationToken cancellationToken)
    {
        var returnOrders = _context.ReturnOrders.OrderBy(c => c.CreatedAt).Where(c => c.ReturnOrderStateHistory.OrderBy(e => e.Created).Last().ReturnOrderStatus == ReturnOrderStatus.Sent);
