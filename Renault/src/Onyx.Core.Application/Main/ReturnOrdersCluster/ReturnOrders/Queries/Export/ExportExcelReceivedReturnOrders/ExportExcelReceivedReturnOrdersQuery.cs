...
public record ExportExcelReceivedReturnOrdersQuery() : IRequest<byte[]>
{
...
public class ExportExcelReceivedReturnOrdersQueryHandler : IRequestHandler<ExportExcelReceivedReturnOrdersQuery, byte[]>
{
    private readonly IApplicationDbContext _context;
    private readonly IExportServices _exportServices;


    public ExportExcelReceivedReturnOrdersQueryHandler(IApplicationDbContext context, IExportServices exportServices)
    {
        _context = context;
        _exportServices = exportServices;
    }

    public async Task<byte[]> Handle(ExportExcelReceivedReturnOrdersQuery request, CancellationToken cancellationToken)
    {
        var returnOrders = _context.ReturnOrders
            .Where(c => c.ReturnOrderStateHistory.OrderBy(e => e.Created).Last().ReturnOrderStatus == ReturnOrderStatus.Received)
            .OrderBy(x => x.Number);
