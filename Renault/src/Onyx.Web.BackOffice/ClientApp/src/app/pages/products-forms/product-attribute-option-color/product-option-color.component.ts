...
import { ProductOptionColorsGridService } from "./product-option-color-grid.service";
import { NgbAlertConfig, NgbModal, NgbModalRef } from "@ng-bootstrap/ng-bootstrap";
import { FormBuilder, FormGroup, UntypedFormBuilder, Validators } from "@angular/forms";
import { ProductOptionColorModel } from "./product-option-color.model";
import { Observable } from "rxjs";
import { Component, QueryList, ViewChild, ViewChildren } from "@angular/core";
import { NgbdProductOptionColorSortableHeader, SortEvent } from "./product-option-color-sortable.directive";
import { ProductOptionColorsClient, UpdateProductOptionColorCommand } from "src/app/web-api-client";
import Swal from "sweetalert2";
import { ExportFileService } from "src/app/shared/services/export-file/export-file.service";

@Component({
  selector: "app-product-option-color",
  templateUrl: "./product-option-color.component.html",
  styleUrls: ["./product-option-color.component.scss"],
  providers: [ProductOptionColorsGridService, DecimalPipe, NgbAlertConfig],
})
export class ProductAttributeOptionColorComponent {
  // bread crumb items
  ...
  selectedItem?: ProductOptionColorModel;
  gridjsList$!: Observable<ProductOptionColorModel[]>;
  total$: Observable<number>;
  inProgress = false;
  inProgressAllExportbtn = false;

  @ViewChildren(NgbdProductOptionColorSortableHeader)
  brandTypes!: QueryList<NgbdProductOptionColorSortableHeader>;
  ...
  @ViewChild("optionColorModal") optionColorModal: any = [];
  ...
  constructor(
    ...
    public service: ProductOptionColorsGridService,
    public exportService: ExportFileService,
    private formBuilder: UntypedFormBuilder
  ) {
    this.gridjsList$ = service.optionColor$;
  ...
  onSubmit(): void {
    ...
    if (this.myForm.valid) {
      if (this.selectedId > 0) {
        ...
        let cmd = new UpdateProductOptionColorCommand();
        ...
        this.client.update(this.myForm.controls["id"].value, cmd).subscribe(
          (result) => {
            ...
            if (result == null) {
              this.service.getAllOptionColors();
              Swal.fire({
                title: "ذخیره آپشن رنگ با موفقیت انجام شد",
                icon: "success",
                iconHtml: "!",
                confirmButtonText: "تایید",
              });
              ...
              Swal.fire({
                title: "ذخیره آپشن رنگ با خطا مواجه شد",
                icon: "error",
                iconHtml: "!",
                confirmButtonText: "تایید",
              });
  ...
  openMoreDetail(item: any) {
    this.selectedOptionColor = item;
    this.modalService.open(this.optionColorModal, {
      size: "lg",
      backdrop: "static",
    });
  }
  ...
  deleteItems() {
    ...
    this.client.deleteRange([...this.checkedItems]).subscribe(
      (result) => {
        ...
        if (result == null) {
          Swal.fire({
            title: "حذف آپشن رنگ با موفقیت انجام شد",
            icon: "success",
            iconHtml: "!",
            confirmButtonText: "تایید",
          });
          ...
          Swal.fire({
            title: "حذف آپشن رنگ با خطا مواجه شد",
            icon: "success",
            iconHtml: "!",
            confirmButtonText: "تایید",
          });
