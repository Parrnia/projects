...
public record ExportExcelCostRefundedOrdersQuery() : IRequest<byte[]>
{
...
public class ExportExcelCostRefundedOrdersQueryHandler : IRequestHandler<ExportExcelCostRefundedOrdersQuery, byte[]>
{
    private readonly IApplicationDbContext _context;
    private readonly IExportServices _exportServices;


    public ExportExcelCostRefundedOrdersQueryHandler(IApplicationDbContext context, IExportServices exportServices)
    {
        _context = context;
        _exportServices = exportServices;
    }

    public async Task<byte[]> Handle(ExportExcelCostRefundedOrdersQuery request, CancellationToken cancellationToken)
    {
        var orders = _context.Orders
            .Where(c => c.OrderStateHistory.OrderBy(e => e.Created).Last().OrderStatus == OrderStatus.OrderCostRefunded)
