...
public record ExportExcelPaymentConfirmedOrdersQuery() : IRequest<byte[]>
{
...
public class ExportExcelPaymentConfirmedOrdersQueryHandler : IRequestHandler<ExportExcelPaymentConfirmedOrdersQuery, byte[]>
{
    private readonly IApplicationDbContext _context;
    private readonly IExportServices _exportServices;


    public ExportExcelPaymentConfirmedOrdersQueryHandler(IApplicationDbContext context, IExportServices exportServices)
    {
        _context = context;
        _exportServices = exportServices;
    }

    public async Task<byte[]> Handle(ExportExcelPaymentConfirmedOrdersQuery request, CancellationToken cancellationToken)
    {
        var orders = _context.Orders
            .Where(c => c.OrderStateHistory.OrderBy(e => e.Created).Last().OrderStatus == OrderStatus.OrderPaymentConfirmed)
