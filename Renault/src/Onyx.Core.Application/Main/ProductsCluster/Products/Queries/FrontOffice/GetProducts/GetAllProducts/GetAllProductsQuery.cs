...
public record GetAllProductsQuery : IRequest<List<AllProductDto>>
{
    public CustomerTypeEnum CustomerTypeEnum { get; set; }
}

public class GetAllProductsQueryHandler : IRequestHandler<GetAllProductsQuery, List<AllProductDto>>
{
    private readonly ISharedProductQueryHelperMethods _productQueryHelperMethods;
    private readonly IMapper _mapper;

    public GetAllProductsQueryHandler(IMapper mapper, ISharedProductQueryHelperMethods productQueryHelperMethods)
    {
        _mapper = mapper;
        _productQueryHelperMethods = productQueryHelperMethods;
    }

    public async Task<List<AllProductDto>> Handle(GetAllProductsQuery request, CancellationToken cancellationToken)
    {
        var products = _productQueryHelperMethods.GetProducts(c => c.IsActive);

        var helperResult = await products.ProjectTo<AllProductModelHelperDto>(_mapper.ConfigurationProvider)
