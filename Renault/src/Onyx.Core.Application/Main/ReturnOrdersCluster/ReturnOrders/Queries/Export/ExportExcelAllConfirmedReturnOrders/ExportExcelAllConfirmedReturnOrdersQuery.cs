...
public record ExportExcelAllConfirmedReturnOrdersQuery() : IRequest<byte[]>
{
...
public class ExportExcelAllConfirmedReturnOrdersQueryHandler : IRequestHandler<ExportExcelAllConfirmedReturnOrdersQuery, byte[]>
{
    private readonly IApplicationDbContext _context;
    private readonly IExportServices _exportServices;


    public ExportExcelAllConfirmedReturnOrdersQueryHandler(IApplicationDbContext context, IExportServices exportServices)
    {
        _context = context;
        _exportServices = exportServices;
    }

    public async Task<byte[]> Handle(ExportExcelAllConfirmedReturnOrdersQuery request, CancellationToken cancellationToken)
    {
        var returnOrders = _context.ReturnOrders
            .Where(c => c.ReturnOrderStateHistory.OrderBy(e => e.Created).Last().ReturnOrderStatus == ReturnOrderStatus.AllConfirmed)
            .OrderBy(x => x.Number);
