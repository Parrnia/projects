...
public record GetTopRatedProductsWithPaginationQuery : IRequest<PaginatedList<TopRatedProductWithPaginationDto>>
{
    public int PageNumber { get; init; } = 1;
    public int PageSize { get; init; } = 10;
    public CustomerTypeEnum CustomerTypeEnum { get; set; }
}
public class GetTopRatedProductsWithPaginationQueryHandler : IRequestHandler<GetTopRatedProductsWithPaginationQuery, PaginatedList<TopRatedProductWithPaginationDto>>
{
    private readonly ISharedProductQueryHelperMethods _productQueryHelperMethods;
    private readonly IMapper _mapper;

    public GetTopRatedProductsWithPaginationQueryHandler(IMapper mapper, ISharedProductQueryHelperMethods productQueryHelperMethods)
    {
        _mapper = mapper;
        _productQueryHelperMethods = productQueryHelperMethods;
    }

    public async Task<PaginatedList<TopRatedProductWithPaginationDto>> Handle(GetTopRatedProductsWithPaginationQuery request, CancellationToken cancellationToken)
    {
        var products = _productQueryHelperMethods.GetProducts(c => c.IsTopRated);

        var helperResult = await products
            .ProjectTo<TopRatedProductWithPaginationModelHelperDto>(_mapper.ConfigurationProvider)
        ...
        var finalResult = new PaginatedList<TopRatedProductWithPaginationDto>(result, helperResult.TotalCount, request.PageNumber, request.PageSize);
        return finalResult;
    }
}
