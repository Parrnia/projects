...
public record GetProductsWithBrandIdWithPaginationQuery : IRequest<PaginatedList<ProductDto>>
{
...
public class GetProductsWithBrandIdWithPaginationQueryHandler : IRequestHandler<GetProductsWithBrandIdWithPaginationQuery, PaginatedList<ProductDto>>
{
    private readonly IApplicationDbContext _context;
    private readonly IMapper _mapper;

    public GetProductsWithBrandIdWithPaginationQueryHandler(IApplicationDbContext context, IMapper mapper)
    {
        _context = context;
        _mapper = mapper;
    }

    public async Task<PaginatedList<ProductDto>> Handle(GetProductsWithBrandIdWithPaginationQuery request, CancellationToken cancellationToken)
    {
        var products = _context.Products
            .Include(c => c.AttributeOptions)
            .ThenInclude(c => c.ProductAttributeOptionRoles)
            .Where(x => x.ProductBrandId == request.BrandId)
            .OrderBy(x => x.Name);
