...
public record ExportExcelTeamMembersQuery() : IRequest<byte[]>
{
...
public class ExportExcelTeamMembersQueryHandler : IRequestHandler<ExportExcelTeamMembersQuery, byte[]>
{
    private readonly IApplicationDbContext _context;
    private readonly IExportServices _exportServices;


    public ExportExcelTeamMembersQueryHandler(IApplicationDbContext context, IExportServices exportServices)
    {
        _context = context;
        _exportServices = exportServices;
    }

    public async Task<byte[]> Handle(ExportExcelTeamMembersQuery request, CancellationToken cancellationToken)
    {
        var teamMembers = _context.TeamMembers
            .OrderBy(x => x.Name);
        
        var selectedProperties = new List<Expression<Func<TeamMember, object>>?>()
        {
            c => c.Name,
            c => c.Position,
            c => c.AboutUs,
