...
public record GetBestSellerProductsWithPaginationQuery : IRequest<PaginatedList<BestSellerProductWithPaginationDto>>
{
    public int PageNumber { get; init; } = 1;
    public int PageSize { get; init; } = 10;
    public CustomerTypeEnum CustomerTypeEnum { get; set; }
}
public class GetBestSellerProductsWithPaginationQueryHandler : IRequestHandler<GetBestSellerProductsWithPaginationQuery, PaginatedList<BestSellerProductWithPaginationDto>>
{
    private readonly IMapper _mapper;
    private readonly ISharedProductQueryHelperMethods _productQueryHelperMethods;


    public GetBestSellerProductsWithPaginationQueryHandler(IMapper mapper, ISharedProductQueryHelperMethods productQueryHelperMethods)
    {
        _mapper = mapper;
        _productQueryHelperMethods = productQueryHelperMethods;
    }

    public async Task<PaginatedList<BestSellerProductWithPaginationDto>> Handle(GetBestSellerProductsWithPaginationQuery request, CancellationToken cancellationToken)
    {
        var products = _productQueryHelperMethods.GetProducts(c => c.IsBestSeller);


        var helperResult = await products.ProjectTo<BestSellerProductWithPaginationModelHelperDto>(_mapper.ConfigurationProvider)
        ...
        var finalResult = new PaginatedList<BestSellerProductWithPaginationDto>(result, helperResult.TotalCount, request.PageNumber, request.PageSize);
        return finalResult;
    }
}
