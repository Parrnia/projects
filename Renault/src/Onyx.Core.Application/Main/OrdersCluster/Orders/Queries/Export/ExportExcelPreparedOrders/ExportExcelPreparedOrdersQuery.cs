...
namespace Onyx.Application.Main.OrdersCluster.Orders.Queries.Export.ExportExcelPreparedOrders;

public record ExportExcelPreparedOrdersQuery() : IRequest<byte[]>
{
...
public class ExportExcelPreparedOrdersQueryHandler : IRequestHandler<ExportExcelPreparedOrdersQuery, byte[]>
{
    private readonly IApplicationDbContext _context;
    private readonly IExportServices _exportServices;


    public ExportExcelPreparedOrdersQueryHandler(IApplicationDbContext context, IExportServices exportServices)
    {
        _context = context;
        _exportServices = exportServices;
    }

    public async Task<byte[]> Handle(ExportExcelPreparedOrdersQuery request, CancellationToken cancellationToken)
    {
        var orders = _context.Orders
            .Where(c => c.OrderStateHistory.OrderBy(e => e.Created).Last().OrderStatus == OrderStatus.OrderPrepared)
