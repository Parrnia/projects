...
public record ExportExcelCanceledReturnOrdersQuery() : IRequest<byte[]>
{
...
public class ExportExcelCanceledReturnOrdersQueryHandler : IRequestHandler<ExportExcelCanceledReturnOrdersQuery, byte[]>
{
    private readonly IApplicationDbContext _context;
    private readonly IExportServices _exportServices;


    public ExportExcelCanceledReturnOrdersQueryHandler(IApplicationDbContext context, IExportServices exportServices)
    {
        _context = context;
        _exportServices = exportServices;
    }

    public async Task<byte[]> Handle(ExportExcelCanceledReturnOrdersQuery request, CancellationToken cancellationToken)
    {
        var returnOrders = _context.ReturnOrders
            .Where(c => c.ReturnOrderStateHistory.OrderBy(e => e.Created).Last().ReturnOrderStatus == ReturnOrderStatus.Canceled)
            .OrderBy(x => x.Number);
