...
export class UrlService {
    ...
    product(product: Product): string {
        debugger;
        return `/shop/products/${product.slug}/${product.name}`;
    }
    ...
    orderPayment(order: Order): string {
        return `/account/orderpayment/${order.id}`;
    }

    orderAnonymous(): string {
        let url = '/account/orders-anonymous';
        return url;
    }
