...
public record GetCanceledOrdersWithPaginationQuery : IRequest<FilteredOrderDto>
{
...
public class GetCanceledOrdersWithPaginationQueryHandler : IRequestHandler<GetCanceledOrdersWithPaginationQuery, FilteredOrderDto>
{
    private readonly IApplicationDbContext _context;
    private readonly IMapper _mapper;

    public GetCanceledOrdersWithPaginationQueryHandler(IApplicationDbContext context, IMapper mapper)
    {
        _context = context;
        _mapper = mapper;
    }

    public async Task<FilteredOrderDto> Handle(GetCanceledOrdersWithPaginationQuery request, CancellationToken cancellationToken)
    {
        var orders = _context.Orders.OrderBy(c => c.CreatedAt).Where(c => c.OrderStateHistory.OrderBy(e => e.Created).Last().OrderStatus == OrderStatus.OrderCanceled);
