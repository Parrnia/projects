...
import { AllConfirmedReturnOrderGridService } from './all-confirmed-return-order-grid.service';
import { NgbdAllConfirmedReturnOrderSortableHeader, SortEvent } from './all-confirmed-return-order-sortable.directive';
...
@Component({
  selector: "app-all-confirmed-return-order",
  templateUrl: "./all-confirmed-return-order.component.html",
  styleUrls: ["./all-confirmed-return-order.component.scss"],
  providers: [AllConfirmedReturnOrderGridService, DecimalPipe, NgbAlertConfig],
})
export class AllConfirmedReturnOrderComponent {
  ...
  @ViewChildren(NgbdAllConfirmedReturnOrderSortableHeader)
  returnOrders!: QueryList<NgbdAllConfirmedReturnOrderSortableHeader>;
  ...
  constructor(
    ...
    public service: AllConfirmedReturnOrderGridService,
  ...
  ngOnInit(): void {
    this.breadCrumbItems = [
      { label: 'سفارش بازگشت' },
      { label: 'لیست سفارشات بازگشت با همه کالاها تاییدشده ', active: true }
    ];
  ...
  exportExcel() {
    ...
    this.client
      .exportExcelAllConfirmedQuery(null, 1, null, null, null, null, null)
      .subscribe({
        next: (response) => {
