...
export class FakeVehicleApi extends VehicleApi {
    ...
    getMakes(year: string): Observable<string[]> {
        return getMakes(year);
    }

    getModels(year: string, make: string): Observable<string[]> {
        return getModels(year, make);
    }

    getVehicles(year: string, make: string, model: string): Observable<Vehicle[]> {
