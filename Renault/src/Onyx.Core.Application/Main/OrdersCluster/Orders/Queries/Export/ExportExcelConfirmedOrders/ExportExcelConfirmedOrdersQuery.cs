...
namespace Onyx.Application.Main.OrdersCluster.Orders.Queries.Export.ExportExcelConfirmedOrders;

public record ExportExcelConfirmedOrdersQuery() : IRequest<byte[]>
{
...
public class ExportExcelConfirmedOrdersQueryHandler : IRequestHandler<ExportExcelConfirmedOrdersQuery, byte[]>
{
    private readonly IApplicationDbContext _context;
    private readonly IExportServices _exportServices;


    public ExportExcelConfirmedOrdersQueryHandler(IApplicationDbContext context, IExportServices exportServices)
    {
        _context = context;
        _exportServices = exportServices;
    }

    public async Task<byte[]> Handle(ExportExcelConfirmedOrdersQuery request, CancellationToken cancellationToken)
    {
        var orders = _context.Orders
            .Where(c => c.OrderStateHistory.OrderBy(e => e.Created).Last().OrderStatus == OrderStatus.OrderConfirmed)
