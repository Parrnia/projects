...
public record GetRejectedReturnOrdersWithPaginationQuery : IRequest<FilteredReturnOrderDto>
{
...
public class GetRejectedReturnOrdersWithPaginationQueryHandler : IRequestHandler<GetRejectedReturnOrdersWithPaginationQuery, FilteredReturnOrderDto>
{
    private readonly IApplicationDbContext _context;
    private readonly IMapper _mapper;

    public GetRejectedReturnOrdersWithPaginationQueryHandler(IApplicationDbContext context, IMapper mapper)
    {
        _context = context;
        _mapper = mapper;
    }

    public async Task<FilteredReturnOrderDto> Handle(GetRejectedReturnOrdersWithPaginationQuery request, CancellationToken cancellationToken)
    {
        var returnOrders = _context.ReturnOrders.OrderBy(c => c.CreatedAt).Where(c => c.ReturnOrderStateHistory.OrderBy(e => e.Created).Last().ReturnOrderStatus == ReturnOrderStatus.Rejected);
