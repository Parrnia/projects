...
import { CountryDto, CountriesClient, CorporationInfosClient, AddLocationAddressCommand, RemoveLocationAddressCommand } from "src/app/web-api-client";
import { LocationAddressGridService } from "./location-address-grid.service";
import { LocationAddressModel } from "./location-address.model";
import { ImageService } from "src/app/core/services/image.service";
// import { NgbdLocationAddressSortableHeader, SortEvent } from "./addresses-sortable.directive";

@Component({
  selector: 'app-location-address',
  templateUrl: './location-address.component.html',
  styleUrls: ['./location-address.component.scss'],
  providers: [LocationAddressGridService, DecimalPipe, NgbAlertConfig],
})
export class LocationAddressComponent {
  ...
  selectedId: any = 0;
  submit!: boolean;
  selectedItem?: LocationAddressModel;
  gridjsList$!: Observable<LocationAddressModel[]>;
  ...
  constructor(
    ...
    public service: LocationAddressGridService,
    private formBuilder: UntypedFormBuilder,
    public client: CorporationInfosClient) {
    this.gridjsList$ = service.locationAddresses$;
  ...
  ngOnInit(): void {
    this.client.getCorporationInfoById(this.corporationInfo.id).subscribe({
      next: (res) => {
        this.service.setLocationAddresses(res.locationAddresses ?? []);
      }
    });
  ...
  onSubmit(): void {
    ...
    if (this.myForm.valid) {
      this.client.addLocationAddress(
...
deleteItems() {
  this.inProgress = true;
  let cmd = new RemoveLocationAddressCommand();
  cmd.corporationInfoId = this.corporationInfo.id;
  cmd.locationAddresses = [...this.checkedItems];
  this.client.removeLocationAddress(this.corporationInfo.id, cmd).subscribe(result => {
    this.inProgress = false;
    if (result == null) {
