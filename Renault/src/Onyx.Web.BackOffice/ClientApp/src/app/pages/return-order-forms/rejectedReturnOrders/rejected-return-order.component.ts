...
import { RejectedReturnOrderGridService } from './rejected-return-order-grid.service';
import { NgbdRejectedReturnOrderSortableHeader, SortEvent } from './rejected-return-order-sortable.directive';
...
@Component({
  selector: "app-rejected-return-order",
  templateUrl: "./rejected-return-order.component.html",
  styleUrls: ["./rejected-return-order.component.scss"],
  providers: [RejectedReturnOrderGridService, DecimalPipe, NgbAlertConfig],
})
export class RejectedReturnOrderComponent {
  ...
  @ViewChildren(NgbdRejectedReturnOrderSortableHeader)
  returnOrders!: QueryList<NgbdRejectedReturnOrderSortableHeader>;
  ...
  constructor(
    ...
    public service: RejectedReturnOrderGridService,
  ...
  ngOnInit(): void {
    this.breadCrumbItems = [
      { label: 'سفارش بازگشت' },
      { label: 'لیست سفارشات بازگشت ردشده', active: true }
    ];
  }
  ...
  exportExcel() {
    ...
    this.client
      .exportExcelRejectedQuery(null, 1, null, null, null, null, null)
      .subscribe({
        next: (response) => {
