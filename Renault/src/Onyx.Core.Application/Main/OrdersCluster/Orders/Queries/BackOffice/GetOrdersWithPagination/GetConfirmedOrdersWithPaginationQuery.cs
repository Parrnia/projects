...
public record GetConfirmedOrdersWithPaginationQuery : IRequest<FilteredOrderDto>
{
...
public class GetConfirmedOrdersWithPaginationQueryHandler : IRequestHandler<GetConfirmedOrdersWithPaginationQuery, FilteredOrderDto>
{
    private readonly IApplicationDbContext _context;
    private readonly IMapper _mapper;

    public GetConfirmedOrdersWithPaginationQueryHandler(IApplicationDbContext context, IMapper mapper)
    {
        _context = context;
        _mapper = mapper;
    }

    public async Task<FilteredOrderDto> Handle(GetConfirmedOrdersWithPaginationQuery request, CancellationToken cancellationToken)
    {
        var orders = _context.Orders.OrderBy(c => c.CreatedAt).Where(c => c.OrderStateHistory.OrderBy(e => e.Created).Last().OrderStatus == OrderStatus.OrderConfirmed);
