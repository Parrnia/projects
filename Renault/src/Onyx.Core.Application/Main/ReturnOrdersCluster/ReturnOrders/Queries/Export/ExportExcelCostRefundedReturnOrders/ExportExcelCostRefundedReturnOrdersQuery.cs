...
public record ExportExcelCostRefundedReturnOrdersQuery() : IRequest<byte[]>
{
...
public class ExportExcelCostRefundedReturnOrdersQueryHandler : IRequestHandler<ExportExcelCostRefundedReturnOrdersQuery, byte[]>
{
    private readonly IApplicationDbContext _context;
    private readonly IExportServices _exportServices;


    public ExportExcelCostRefundedReturnOrdersQueryHandler(IApplicationDbContext context, IExportServices exportServices)
    {
        _context = context;
        _exportServices = exportServices;
    }

    public async Task<byte[]> Handle(ExportExcelCostRefundedReturnOrdersQuery request, CancellationToken cancellationToken)
    {
        var returnOrders = _context.ReturnOrders
            .Where(c => c.ReturnOrderStateHistory.OrderBy(e => e.Created).Last().ReturnOrderStatus == ReturnOrderStatus.CostRefunded)
            .OrderBy(x => x.Number);
