...
public record ExportExcelRegisteredReturnOrdersQuery() : IRequest<byte[]>
{
...
public class ExportExcelRegisteredReturnOrdersQueryHandler : IRequestHandler<ExportExcelRegisteredReturnOrdersQuery, byte[]>
{
    private readonly IApplicationDbContext _context;
    private readonly IExportServices _exportServices;


    public ExportExcelRegisteredReturnOrdersQueryHandler(IApplicationDbContext context, IExportServices exportServices)
    {
        _context = context;
        _exportServices = exportServices;
    }

    public async Task<byte[]> Handle(ExportExcelRegisteredReturnOrdersQuery request, CancellationToken cancellationToken)
    {
        var returnOrders = _context.ReturnOrders
            .Where(c => c.ReturnOrderStateHistory.OrderBy(e => e.Created).Last().ReturnOrderStatus == ReturnOrderStatus.Registered)
            .OrderBy(x => x.Number);
