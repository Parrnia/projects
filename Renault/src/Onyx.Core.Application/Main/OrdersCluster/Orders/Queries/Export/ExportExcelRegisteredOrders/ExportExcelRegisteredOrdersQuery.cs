...
public record ExportExcelRegisteredOrdersQuery() : IRequest<byte[]>
{
...
public class ExportExcelRegisteredOrdersQueryHandler : IRequestHandler<ExportExcelRegisteredOrdersQuery, byte[]>
{
    private readonly IApplicationDbContext _context;
    private readonly IExportServices _exportServices;


    public ExportExcelRegisteredOrdersQueryHandler(IApplicationDbContext context, IExportServices exportServices)
    {
        _context = context;
        _exportServices = exportServices;
    }

    public async Task<byte[]> Handle(ExportExcelRegisteredOrdersQuery request, CancellationToken cancellationToken)
    {
        var orders = _context.Orders
            .Where(c => c.OrderStateHistory.OrderBy(e => e.Created).Last().OrderStatus == OrderStatus.OrderRegistered)
